        ...
        >
            <ul class="space-y-2">
                <li>
                    ...
                    >
                        <span class="ml-3 text-left text-lg">{{
                            croatianCounties[county]
                        }}</span>
                    </a>
                </li>
                <hr />
                ...
                <li v-for="city in cities" :key="city">
                    <div class="flex items-center mb-4">
                        <input
                            ...
                            @change="updateSelectedCities(city)"
...
export default {
    props: {
        ...
        //selectedCounties: { type: Array, required: true },
        //gradoviZupanije: { type: Object, required: true },
    },
    data() {
        return {
            selectedCities: [],
    ...
    methods: {
        ...
        /*
        updateSelectedCounty(county, isSelected) {
            const citiesInCounty = this.gradoviZupanije[county]; // Assuming `cities` prop contains cities of the selected county
            console.log(citiesInCounty);
            if (isSelected) {
                this.selectedCities = this.selectedCities.filter(
                    (city) => !citiesInCounty.includes(city)
                );
            }
        },*/
    },
    /*watch: {
        selectedCounties(newCounty, oldCounty) {
            const oldCounty2 = oldCounty.find(
                (county) => !newCounty.includes(county)
            );
            console.log(oldCounty2);
            if (oldCounty2) {
                //this.updateSelectedCounty(oldCounty2, false);
            }
        },
    },*/
};
